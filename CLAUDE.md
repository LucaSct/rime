...
C++20 engine (moving to C++23 features where stable) built with CMake; Rust tooling built
with Cargo. The manifests are the source of truth for versions and dependencies. What they
cannot tell you:
...
### C++ (engine/)
- Formatting is enforced by [`.clang-format`](.clang-format) — run it; don't hand-format.
...
Every C++/Rust source file opens with the SPDX Apache-2.0 header — copy it from any
neighbouring file; `scripts/check-license-headers.sh` gates it in CI.
...
Per-brick: own branch → build → lavapipe green → small commits → **clang-format** → push → PR →
CI (3-OS + format + ASan/UBSan + TSan) → merge.

**Never push without running clang-format over CI's exact file set** — `find`, not
`git ls-files`, which skips untracked new files and so silently misses a brand-new source file:
`~/.rime-tools/bin/clang-format -i $(find engine tests \( -name '*.cpp' -o -name '*.hpp' -o -name
'*.mm' \))`. CI's format job is the backstop, not the first line of defence; skipping the local
run cost M6.3, M6.4 and M6.8 a red-CI round-trip each.

The full procedure — the `docs/perf/` requirement for perf-touching bricks, driving PRs through
the REST API rather than the `gh` wrappers, and the stacked-PR rebase every child needs the moment
its parent squash-merges — is the **`brick-delivery` skill**. Read it before pushing a stack.
