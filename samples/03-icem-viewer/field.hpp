...
//
// Reads ICEM's native .icef field file and prepares a scalar field for the viewer: a dense node grid
// of values, packed into an RGBA32F volume (R = value, G = validity) for a 3-D texture, plus the
// affine map from world position to texture coordinate and the value range for the colormap.
//
// The two repos share only the *file format*, never code — this is an independent reader of the
// little-endian layout documented in ICEM's docs/math/field-io.md (and Rime's docs/math/colormap.md):
//   magic "ICEF" | u32 version(=1) | u32 flags | origin xyz f64 | h f64 | nx,ny,nz u32 (node counts)
//   | u32 field_count | per field: name(u32 len+bytes) unit(str) components(i32) then
...
struct ScalarField {
    ...
    std::vector<float> rgba; // nx*ny*nz * 4: R = value (dilated into the absent shell), G = valid(0/1)
...
// Load the first scalar (1-component) field from a .icef — or the one named `want`, if given. Returns
// nullopt on I/O error, bad magic/version, or if no matching scalar field exists.
[[nodiscard]] inline std::optional<ScalarField> load_icef_scalar(const std::string& path,
    ...
    if (detail::rd_u32(is) != 1u) return std::nullopt; // version
    ...
    for (std::size_t i = 0; i < count; ++i) {
        if (!mask[i] || std::isnan(chosen[i])) continue;
    ...
    if (sf.vmax <= sf.vmin) sf.vmax = sf.vmin + 1.0f; // constant field → a non-degenerate domain
    ...
    // instead of a fill constant, so the surface colour reads true right up to the edge. The validity
    // channel keeps the *original* mask (the true solid), for the slice clip that lands with the cap.
