...
//
// Proof for the gas-path chart (D4) — the from-scratch 2-D line plot of a computed scalar field
// along the flow axis. Three halves:
//   * sampling (CPU, always): the axial-profile sampler picks the longest grid axis and averages
//   each
//     cross-section slice, reproducing a known Mach distribution (inlet subsonic → exit
//     supersonic).
//   * layout (CPU, always): build_gas_path_chart emits geometry (panel + axes + curve), and more of
//   it
//     when the field crosses M = 1 (the sonic reference line is drawn).
//   * render (off-screen, GPU): the chart draws — panel fill, the amber curve, bright axis labels.
//   Set
...
// A synthetic nozzle Mach field: a slender grid (long along z), Mach rising linearly from a
// subsonic inlet to a supersonic exit, every voxel valid — the shape brick26 produces, without
// needing ICEM.
        ...
        for (std::uint32_t j = 0; j < f.ny; ++j)
            for (std::uint32_t i = 0; i < f.nx; ++i) {
                const std::size_t g = (static_cast<std::size_t>(i) +
                                       f.nx * (j + static_cast<std::size_t>(f.ny) * k)) *
                                      4;
...
TEST_CASE("chart: the layout emits geometry, and the sonic line only when M=1 is in range (D4)") {
    ...
    // The same field clamped to a fully-supersonic range never crosses M = 1 → no sonic reference
    // line, so strictly fewer vertices than the crossing case above.
