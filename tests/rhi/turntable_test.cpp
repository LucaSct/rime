...
//
// Proof for the turntable export's novel logic (E4): the per-frame camera yaw sequence and the
// numbered frame file naming. These are the only new pieces — each frame's actual render reuses the
// proven off-screen path (covered by mesh_offscreen_test / cap_offscreen_test), so this stays
// CPU-only and always runs in CI. See turntable.hpp and docs/math/orbit-camera.md.
...
TEST_CASE("turntable: the yaw sweep is a full, evenly-spaced 360° from the start pose (E4)") {
    ...
    // n is clamped to ≥ 1, so a degenerate count can't divide by zero (frame 1 lands a full turn
    // on).
...
TEST_CASE("turntable: the frame path inserts a zero-padded index on the template's stem (E4)") {
    ...
    CHECK(turntable_frame_path("icem_view.ppm", 123) ==
          "icem_view_123.ppm"); // padding doesn't clip
