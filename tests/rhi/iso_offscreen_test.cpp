...
//
// Proof for the viewer's raymarched isosurface / DVR (C2). A unit cube holds a scalar field that ramps
// along z (0 at the bottom → 1 at the top); render_iso_offscreen marches it for two isovalues and for a
// DVR. Assertions: each isosurface appears (sensible coverage); the low isovalue (z near the bottom) is
// cold-blue and the high one hot-red, so the isotherm sits where the colormap says it should; and the
// DVR composites the whole volume (more coverage than a single isosurface). Off-screen + readback,
// GPU-free on lavapipe in CI.
...
TEST_CASE("viewer raymarches an isosurface and a DVR of a scalar field (C2)") {
    ...
    const auto render = [&](float iso, bool dvr) {
        return rime::viewer::render_iso_offscreen(
            *device, size, cube, make_push(iso, dvr), clear, mesh_vert_spv, sizeof(mesh_vert_spv),
            mesh_frag_spv, sizeof(mesh_frag_spv), iso_vert_spv, sizeof(iso_vert_spv), iso_frag_spv,
            sizeof(iso_frag_spv), vol.data(), 2, 2, 2);
    };
    ...
    CHECK(lo.part > 100);
    CHECK(hi.part > 100);
    // The low isovalue reads cold (blue dominant), the high one hot (red dominant): the isotherm sits
    // where the colormap places that value.
    CHECK(lo.b / static_cast<long>(lo.part) > lo.r / static_cast<long>(lo.part));   // blue > red (cold)
    CHECK(hi.r / static_cast<long>(hi.part) > hi.b / static_cast<long>(hi.part));   // red > blue (hot)
