...
// test texture — top-left red, top-right green, bottom-left blue, bottom-right yellow — plus a
// corner outside the quad showing the dark clear color. Passing proves the whole M3.5 chain ran end
// to end: the index buffer drew the two triangles, the texture upload (staging copy) landed the
// right texels, the sampler read them, and the combined image-sampler descriptor reached the
// fragment shader — i.e. the shader really sampled the texture per pixel. Off-screen, so it runs on
// a software GPU (lavapipe) in CI with no display. (main() for this exe is in device_test.cpp.)
...
TEST_CASE("rhi renders a textured quad off-screen (pixel-verified)") {
    ...
    const std::vector<std::uint8_t> px = rime_sample::render_quad_offscreen(*device,
                                                                            size,
                                                                            clear,
                                                                            quad_vert_spv,
    ...
    // RGBA8, row-major: pixel (x,y) starts at (y*size + x) * 4, channels R,G,B,A. The quad spans
    // NDC
    // [-0.9, 0.9], leaving a ~2-pixel clear border around a ~28×28 quad. Vulkan's clip-space Y
    // points down, so uv (0,0) lands top-left: the quadrants read R,G / B,Y top-to-bottom (see
    // make_quad).
    ...
    // quad each channel reads ~0 or ~255: comparing to the 128 midpoint recovers which color
    // landed.
