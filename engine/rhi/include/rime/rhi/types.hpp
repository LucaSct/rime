...
enum class Format : std::uint32_t {
    ...
    R16Snorm,    // one 16-bit SIGNED-NORMALIZED channel: the GPU decodes the stored int16 as
                 // clamp(int16 / 32767, -1, 1) on sample/imageLoad and encodes the inverse on
                 // imageStore. The SDF clipmap's narrow-band storage (m10.4b, ADR-0032 §10): a
                 // signed distance is always bounded (clamped to a per-level band before storing),
                 // so a normalized 2-byte channel loses nothing a raw f32 would have kept *for that
                 // use*, at a quarter the bytes. Using it as a STORAGE image (imageStore/imageLoad,
              // not just sampling) needs the `shaderStorageImageExtendedFormats` device feature —
              // see the Vulkan backend's device setup.
