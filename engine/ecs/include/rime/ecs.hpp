...
//
// As of M4.4a: entities (generational ids), the entity directory, the component-type registry, the
...
// column-wise iteration over the entities that have a given set of components, and
// Query::par_for_each to run that iteration across all cores on the job system (one chunk per
// task). The inter-system scheduler — declared access sets → phase ordering — lands in M4.4b.
