...
//
// The texture references are AssetIds (content hashes), not paths or inline pixels: a material names
// the *cooked bytes* it needs, and the loader resolves each id to an already-loaded rhi::Texture,
// sharing one GPU upload across every material that references the same image (ADR-0024, decision 2).
// An id of 0 (kInvalidAssetId) means "no texture for this slot"; the renderer binds a 1x1 fallback
// (white / flat-normal / white-AO) so a single shader serves every material permutation (M6.4).
...
// pipeline draws Blend as Opaque for now (an honest limitation — real OIT/sorting is a later brick).
...
// A cooked material in memory. Every factor defaults to the glTF default, so a material that omits a
// property decodes to exactly what glTF specifies. Colours are LINEAR (the cook converts sRGB
