...
| M6.2 | the Rust cooker (`rime-cli cook`) + glTF mesh import → a golden fixture this reader ingests in CI | next |
| M6.3–M6.4 | textures + materials (new `AssetKind`s, same container) | planned |
...
behaviour; `tests/assets/cooked_mesh_test.cpp` drives one crafted file per failure mode (bad magic,
wrong version/kind/schema, truncation at every byte length, size disagreements, a broken vertex
layout, an out-of-range index, a bad submesh) and the suite is ASan/UBSan-clean.
...
Cooked payloads embed a `type_schema_hash`. For meshes it is the reflection `type_hash`
([`core/reflect`](../core/include/rime/core/reflect/type_info.hpp)) of the v1 vertex layout, so if that
layout ever changes the loader rejects old files with a "re-cook" error instead of misreading them.
The same reflection fingerprint later gates M9 inspector compatibility and M11 replication schemas —
...
include/rime/assets/    # public interface (core types only; no rhi/render, no source-format parsers)
  ...
  cooked_reader.hpp     #   the RMA1 container reader + AssetError + mesh_schema_hash()
...
src/
  cooked_reader.cpp     #   header validation + trust-nothing mesh decode (reuses core's byte cursors)
...
- `tests/assets/cooked_mesh_test.cpp` — a valid file round-trips to exactly the written data with the
  right content id; the negative battery refuses every malformed file cleanly.
