...
| m12.6 | the M12 milestone proof — [`samples/13-networked-player`](../../samples/13-networked-player): a server and two clients, the prediction-off control a peer in the same match. **M12 "The Player" closes here.** | landed |
| m13.1 | Track FX brick fx1 — the GPU draw pass for the existing deterministic CPU sim | next (M13) |
