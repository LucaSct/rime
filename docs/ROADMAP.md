...
> and change detection designed in from day one. **M4.1 + M4.2a have since landed** — the
> `engine/ecs` module: the generational entity directory + reflection-aware component registry
> (M4.1), and the allocator-backed chunk storage primitives `ChunkPool` / `ChunkLayout` / `Chunk`
> (M4.2a), ASan-clean. **Next:** M4.2b — the World archetype integration (add/remove component =
> archetype move).
>
