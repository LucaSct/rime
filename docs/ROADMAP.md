...
| **M18** | **virtualized geometry** | Nanite-style rigid opaque meshes: a separately versioned clustered payload, replacement DAG, GPU-driven culling, visibility buffer and hybrid sub-pixel rasterization. Parked as "m10.i" by [ADR-0035](adr/0035-vision-demo-m12.md) §6 and deferred by [ADR-0041](adr/0041-the-visual-bar-m17.md) §3 because it is a milestone, not a brick. [ADR-0043](adr/0043-virtualized-geometry-m18.md) opens it: conventional rendering remains the compatibility path; terrain's heightfield, collision, splat blending and streaming are **M19** under ADR-0041's pre-commitment, rather than silently folded into cluster rendering. |
